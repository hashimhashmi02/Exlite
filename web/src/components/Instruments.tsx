...
export default function Instruments({ assets, onPick, picked }:{
  ...
  const filtered = useMemo(()=> {
    ...
    return rows.filter(r => (r.symbol + r.name).toLowerCase().includes(qq));
  }, [rows,q]);
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="p-3">
        <input value={q} onChange={e=>setQ(e.target.value)}
          placeholder="Search" className="w-full rounded-lg bg-white/10 px-3 py-2 outline-none text-sm"/>
      </div>

      <div className="px-3 pb-2 text-[11px] uppercase text-white/40 grid grid-cols-[1fr,90px,90px] gap-2">
      ...
      <div className="flex-1 overflow-auto px-2 pb-3 space-y-2">
        {filtered.map(a=>(
          <button key={a.symbol} onClick={()=>onPick(a.symbol)}
            className={`w-full grid grid-cols-[1fr,90px,90px] items-center gap-2 px-3 py-2 rounded-xl border transition
            ...
            <div className={`text-right text-sm ${picked===a.symbol?'text-black/70':'text-white/80'}`}>
